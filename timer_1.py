...
class StopWatch(Frame):  
    ...
    def makeWidgets(self):  
        '''''制作时间标签'''  
        if MODE == 0 or MODE == 1:
            l = Label(self, font=("黑体", 260, "bold"), fg='blue', textvariable = self.timestr)  
        else:
            l = Label(self, font=("黑体", 400, "bold"), fg='blue', textvariable = self.timestr)  
    ...
    def stopwatch(self):  
        if self.flag == True:  
            self.pack(side = TOP)  
            Button(self, text = '开始', font=("黑体", 26, "bold"),  command = self.Start).pack(side = LEFT)  
            Button(self, text = '暂停', font=("黑体", 26, "bold"),  command = self.Stop).pack(side = LEFT)  
            Button(self, text = '重置', font=("黑体", 26, "bold"),  command = self.Reset).pack(side = LEFT)  
